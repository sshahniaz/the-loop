...
const ProductsMainContainer = ({ pType }: PContainerProps) => {
          ...
          for (const [filterName, filterValues] of Object.entries(
            ...
            if (filterName === "price") {
              const [minPrice, maxPrice] = filterValues[0]
                ...
                .map(Number);
              match =
                match && product.price >= minPrice && product.price <= maxPrice;
            } else {
  ...
  const handleFilterChange = (filterName: string, value: string) => {
    ...
    setSelectedFilters((prevSelectedFilters) => {
      ...
      return isFilterEmpty
        ? {}
        : { ...prevSelectedFilters, [filterName]: updatedSelection };
    });
  };
  ...
  useEffect(() => {
    ...
    // Only refetch if filters changed significantly (excluding initial render)
    if (
      JSON.stringify(prevFilters.current) !== JSON.stringify(selectedFilters) ||
      !initialProducts
    ) {
      getProducts();
    }
